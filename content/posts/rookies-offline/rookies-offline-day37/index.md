---
title: "[Devlog] SK 쉴더스 루키즈 5기 오프라인 세션 Day 37 — Playwright 증적 스크린샷 (1-2 · 7-4 · 2-2)"
date: 2026-07-10
tags:
  - kdt
  - sk-rookies
...
> **"진단 결과만 YAML로 남기던 아르고스(Argus)에, Playwright로 증적 스크린샷을 자동으로 찍는 후처리 파이프라인을 붙였습니다. 1-2(인젝션)·7-4(CVE/보안설정)에 먼저 올리고, 같은 구조로 2-2(중요 정보 파일 다운로드)까지 확장했습니다. 진단 코드와 캡처 코드를 나누고, 캡처가 실패해도 진단 결과는 깨지지 않게 만들었습니다."**
>
> 증적 캡처 레이어 설계, 진단 후 자동 후처리, 1-2·7-4·2-2 모듈별 캡처 방식과 저장 구조를 정리합니다.
...
- 6-1(오류 페이지 정보 노출) 등 아직 같은 자동 캡처 훅에 안 묶인 섹션을 같은 패턴으로 붙이기
- 캡처 보드·선정 규칙을 가이드라인 항목별로 더 맞추기
- 대시보드에서 evidence 폴더를 보고서·발표 장표로 바로 쓰기 쉽게 연결하기

한 줄로 말하면, 오늘은 **진단이 끝난 뒤 Playwright가 증적을 자동으로 남기는 파이프라인**을 1-2·7-4에 올리고, 2-2까지 같은 후처리에 이은 날입니다.
