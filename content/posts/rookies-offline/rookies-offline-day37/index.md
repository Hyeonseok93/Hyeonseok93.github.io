...
`backend/diagnosis/modules/1-6/` 안에서는 대략 이런 순서로 돌아갑니다.
...
Selenium은 슬림 이미지에서 없으면 import만으로도 백엔드가 죽을 수 있어서, 예전에는 모듈 최상단 import를 빼고 **쓸 때만** 불러오도록 바꿨습니다.
...
문제는 캡처 엔진이 **진단 모듈 안(`screenshot.py` 등)과 `screenshot/modules/1-6/`에 겹쳐** 있던 점이었습니다. 그래서:

1. 진단 모듈에 남아 있던 **중복 Selenium screenshot engine 제거**
2. 실행 진입을 `backend/screenshot/modules/1-6/runner.py` 한곳으로 맞춤
3. capture 설정을 다시 보고, 남아 있던 **오래된/고아 run 산출물 prune**

흐름은 대략 이렇게 보면 됩니다.

```
1-6 진단 실행 (fuzzer → collector → latest.yaml)
        ↓
...
1-2·2-2처럼 `evidence_capture_service`의 자동 후처리 집합(`{"1-2", "2-2", "7-4"}`)에는 **아직 안 넣었습니다.**  
오늘은 runner를 공통 폴더 규격에 맞게 정리하는 쪽이 먼저였고, 자동 훅 연결은 다음입니다.
...
POST /api/diagnosis/cancel           →  DiagnosisCancelled, 지금까지 모은 증적만 latest.yaml에 저장
```
...
`backend/screenshot/modules/6-1/` 아래에 오류 페이지 전용 캡처를 만들고 있습니다. 중심은 `capture.py`이고, 진단이 남긴 Finding(오류 응답 HTML·스택 텍스트 등)을 읽어 **에러 화면 / 응답 보드**를 PNG로 남기는 구조입니다.
...
1-2처럼 “정상 vs 공격” 5장 보드까지는 아직 공통 훅에 묶이지 않았고, **캡처 모듈 본체 작성 단계**입니다. `evidence_capture_service`의 `_AUTO_CAPTURE_SECTIONS`에도 아직 `6-1`이 없어서, 진단 저장 직후 자동 subprocess로는 안 돌아갑니다.
...
1-6·6-1도 같은 `report/{section_id}/evidence/` 아래에 쌓이도록 맞춰 가는 중이고, 자동 훅에 붙으면 위 트리와 같은 형태로 정리됩니다.
