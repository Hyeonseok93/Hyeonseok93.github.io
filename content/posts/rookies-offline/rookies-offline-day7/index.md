---
title: "[Devlog] SK 쉴더스 루키즈 5기 오프라인 세션 Day 7 — 프론트엔드 아키텍처 개선 및 백엔드 통합 시작"
date: 2026-05-28
tags:
  - KDT
  - "SK Rookies"
...
> **"HTML 프로토타입 화면을 컴포넌트 기반 React 구조로 옮겼습니다. 백오피스 관리 기능과 3개 상품(숙소·항공·렌터카)의 통합 예약 흐름도 만들었습니다."**
>
> 오늘은 관리자·판매자 백오피스, 숙소·항공·렌터카 상품 흐름과 통합 예약 모달, 인증·에러·온보딩 UI를 React로 옮기고, 백엔드·인프라 쪽 통합 현황도 함께 정리했습니다.

# 1. 프론트엔드 집중 개발의 결실

지난 6일차에 팀의 개발 속도를 높이기 위해 풀스택 구조에서 **프론트/백엔드 전문화**로 방식을 바꿨습니다. 이에 따라 7일차에는 기존 HTML 프로토타입에 머물러 있던 UI 요소들을 컴포넌트 기반의 **React 구조로 옮기는** 스프린트를 진행했습니다.

이번 작업은 크게 세 부분으로 나뉘었습니다.
...
관리자와 판매자가 시스템을 편하게 관리할 수 있도록 공통 쉘(Shell) 구조인 `BackOfficeLayout.tsx`를 설계하고, 권한별로 나눈 대시보드 패널을 구현했습니다.
...
| | `AdminUserPanel` | 전체 가입 회원 관리 및 제어 |
...
사용자가 상품을 찾고 예약하기까지의 사용자 경험(UX) 패턴을 통일하고, 비즈니스 로직 검증 기능이 있는 **상세 예약 모달**을 보완했습니다.

## 공통 UX 탐색 패턴 표준화

- **검색 전/후 상태 분리:** 검색 전에는 랜덤 추천 리스트와 디폴트 날짜를 보여주고, 사용자가 검색 버튼을 명확히 클릭했을 때만 필터와 날짜 조건이 반영되도록 제어했습니다.
- **통합 서치폼 디자인:** 숙소, 항공권, 렌터카의 서치폼 레이아웃과 디자인 스타일을 하나로 맞춰 보기 쉽게 했습니다.
...
- 회원가입 완료 후 자연스럽게 등장하는 웰컴 팝업(Welcome Popup)의 트랜지션 애니메이션 노출 속도를 **0.2초**로 조정해 컴포넌트 반응을 다듬었습니다.

# 5. 인증 · 회원가입 및 UI/UX 디테일 개선

백오피스 권한 인가의 기초가 되는 **인증 및 온보딩 흐름**을 정리했습니다.
...
- **정보 전달 팝업 구현:** 판매자 회원가입 팝업에서 입력한 가입 신청 데이터가 백엔드로 안전하게 전송되도록 로직을 정리했습니다.
- **승인 대기 모달(`SellerPendingModal.tsx`) 추가:** 판매자가 가입을 완료해도 관리자의 최종 승인 전에는 서비스에 들어가지 못하도록 안내하는 **판매자 승인 대기 팝업**을 만들었습니다. 이 모달은 이후 관리자 페이지의 검수 패널(`AdminHQPanel`)과 연결해 권한 제어에 사용합니다.
...
프론트엔드에서 화면 UI와 클라이언트 비즈니스 로직을 만드는 동안, 백엔드와 인프라 파트도 연동을 위한 작업을 진행했습니다.
...
# 7. 다음 단계: 연동 테스트 및 취약점 진단 준비

기본 UI/UX 구조와 개별 모달 컴포넌트 구현이 마무리되었으므로, 다음 단계부터는 시스템 연결과 취약점 진단이라는 프로젝트 목표에 집중합니다.
