...
> **"자동 진단 플랫폼 아르고스(Argus)의 동적 스캔 컴플라이언스 기준 정립을 위해, 진단 대상 여행 플랫폼 '온데(Onde)'를 타깃으로 대대적인 최초 취약점 전수 진단을 집행했습니다. 이번에는 비즈니스 로직에 엮인 파라미터가 역대급으로 너무나도 많아 매핑하는 데 정말 쉽지 않은 과정을 거쳤습니다. 처음 분석해 보는 대량의 스캔 패킷이다 보니 노하우가 부족해 시간도 상상 이상으로 오래 걸렸고, 예외 처리 필터의 경계선이 모호해 수많은 시행착오와 씨름하며 밤새 작성한 실전 취약점 진단 상세 결과 보고서를 공유합니다."**
>
> 비즈니스 로직에 얽힌 대량의 파라미터를 매핑하며 1-6, 2-1, 2-2, 3-1 항목 진단을 완료했습니다. 초보 진단자의 시행착오와 핵심 결함 요약을 공유합니다.

# 1. 초보 진단자의 솔직한 시행착오 고백

이전의 단순한 주입 공격 검증을 넘어, 웹 애플리케이션의 모든 입력 데이터 크기, 포맷 형식, 무결성 제약조건을 검증하는 단계로 진입하니 말 그대로 '파라미터 지옥'이 펼쳐졌습니다. 각 도메인(회원, 프로필, 커뮤니티, 항공, 숙소, 렌터카, 결제, 보험)마다 요청 DTO에 묶인 인자들이 너무 많아, Burp Suite 프록시로 패킷 구조를 일일이 분석하고 조작하는 요령이 없는 상태에서는 엄청난 시간 병목이 발생했습니다.
...
- **문제점 요약:** 각 비즈니스 API를 처리하는 백엔드 요청 DTO 내부의 입력 필드들에 대해 문자열 최대 길이 및 수치 유효 범위 검증 로직이 전반적으로 누락되어 있습니다. 이로 인해 데이터베이스에 정의된 컬럼 크기나 규격을 초과하는 대용량 데이터, 비정상적인 타입이 주입될 때 사전에 예외 처리를 수행하지 못하고 DB 계층에서 무결성 에러를 일으킵니다. 결과적으로 최상위 에러인 **HTTP 500 내부 서버 오류**가 발생하며 백엔드 내부의 구체적인 SQL 실행 오류 로그 및 시스템 상세 스택 트레이스 기밀 정보가 외부 응답 데이터에 필터링 없이 그대로 노출되는 취약점이 전사 도메인에서 실증되었습니다.
...
- **문제점 요약:** Onde 플랫폼의 이미지 파일 업로드 컴포넌트 호출 시, 파일 명세 확장자 및 실제 바이너리 파일 타입(MIME Type)에 대한 유효성 검증 장치가 유실되어 있습니다. 이로 인해 공격자가 우회 웹셸 스크립트(`.jsp`)나 크로스 사이트 공격 코드가 은닉된 스크립트 파일(`.html`, `.svg`)을 제한 없이 파일 스토리지(S3/MinIO)에 밀어 넣을 수 있으며, 반환되는 퍼블릭 다운로드 경로 주소를 직접 가로채 호출함으로써 원격 코드 실행(RCE) 및 저장형 XSS 공격 체인으로 전개될 수 있는 심각한 결함입니다.
...
서버 측 파일 처리 모듈 단에서 대소문자 구분을 완전히 배제한 강력한 화이트리스트 기반 확장자 검증(이미지 포맷인 JPG, GIF, PNG 등만 허용)을 구현하고, 이중 확장자(`.jpg.jsp`)나 특수문자 기호(`.`)를 이용한 탐제 우회 기법을 철저히 차단해야 합니다. 파일명 내부 경로 조작 및 파싱 오동작을 유유발하는 특수문자 수용을 차단하고, 업로드 파일의 최소/최대 물리 용량 제한(Size Limit)을 설정해야 합니다. 최종 저장 경로는 웹서버 하위 디렉터리가 아닌 완전히 격리된 별도의 물리 드라이브나 전용 독립 스토리지 서버에 분리하고, 해당 공간의 실행 권한(Execute)을 원천 거부 처리해야 합니다.
...
다운로드 및 참조할 파일 경로를 결정 짓는 파라미터 변수에 대해 경로 조작을 유발하는 필터링 대상 특수문자 목록(`../`, `/`, `\`, `%` 등)이 인입되었는지 서버 단에서 유효성 필터를 가해야 합니다. 또한, 클라이언트가 서버 내부의 실제 파일명이나 물리 경로를 직접 주입하지 못하도록 통제하고, 사전에 서버 단에 정의된 안전한 화이트리스트 인덱스 코드 값만을 전달받아 서버 내부에서 매핑 가동하는 '파일 간접 참조 방식'을 전격 채택해야 합니다.
...
이번 최초 수동 동적 진단 스프린트를 통해 수많은 파라미터가 얽힌 모던 웹 서비스의 입력값 검증 영역을 깊숙이 파헤칠 수 있었습니다. 노하우 부족으로 패킷의 유효 인자를 매핑하고 500 에러 이면에 숨겨진 기밀 스택트레이스 유출 결함들을 컴플라이언스 지표와 1:1로 매칭하는 과정에서 많은 고충을 겪었지만, 보안 내재화의 본질을 체화하는 계기가 되었습니다.
