...
> **"진단 대상 여행 플랫폼 '온데(Onde)' 실서버 환경을 타깃으로 첫 번째 수동 패킷 분석 및 웹/API 보안 취약점 진단을 집행했습니다. 생전 처음으로 Burp Suite 프록시 도구를 켜고 날것의 패킷을 가로채며 가이드라인과 대조하다 보니, 노하우가 턱없이 부족하여 단순한 변조 포맷을 맞추는 데도 시간이 무척 오래 걸렸습니다. 특히 서버 예외 처리 분기 등 보안 판단 경계선이 애매모호해 밤새 팀원들과 끙끙 앓으며 도출해 낸 취약점 진단 결과를 기록합니다. 전체 8개 진단 그룹 중 이제 막 '1번 대분류 항목의 일부'를 수행했을 뿐이며, 멘토님의 가이드를 반영해 각 대분류별 '대표 Case 1개만 증적 사진 위치를 지정'하고 나머지는 요약 설명으로 정돈했습니다."**
>
> 온데 실서버를 대상으로 첫 수동 패킷 분석을 집행했습니다. Burp Suite로 가이드라인 1번 대분류 항목을 대조하며 도출한 진단 결과를 정리합니다.

# 1. 초보 진단자의 솔직한 고충과 시행착오

이론으로만 접하던 웹 해킹 취약점을 실제 가동 중인 서비스에 대입하려니 시작부터 장벽에 부딪혔습니다. 프록시 도구의 요청/응답 패킷 구조를 세밀하게 디깅하는 요령이 없다 보니 파라미터를 변조하고 세션을 유지하는 과정에서 막대한 시간을 소비했습니다.

특히 쿼리나 입력창에 악성 구문을 주입했을 때 서버가 **HTTP 500 Internal Server Error**를 뿜어내며 내부 디버그 예외 정보를 유출하는 현상을 마주했을 때, 이를 단순한 개발 로직 에러로 볼 것인지 아니면 가이드라인상의 정보 노출 취약점으로 엄격하게 분류해야 할지 판단 기준이 너무나 애매하여 새벽 내내 논쟁을 벌이기도 했습니다. 거대한 컴플라이언스 체계 앞에서 노하우 부족을 절실히 실감하며, 우선 확인된 결함들을 명확히 요약해 나가는 것부터 시작하기로 했습니다.
...
모든 입/출력 값 검증은 서버 측에서 수행해야 하며, 특수문자(`<`  `&lt;`, `>`  `&gt;`, `(`  `&#40;`, `)`  `&#41;`, `#`  `&#35;`, `&`  `&amp;`, `'`  `&#39;`, `"`  `&quot;`)에 대한 표준 치환 필터를 설정하거나 대소문자 구분 없는 XSS 키워드 비교 함수를 강제 적용해야 합니다.
...
- **문제점 요약:** 서버 측에서 비즈니스 검증 및 인가 프로세스를 누락하여, 클라이언트가 전송하는 고유 파라미터(금액, 마일리지, 식별자 자원) 값을 가로채 임의로 변경하더라도 백엔드가 원천 데이터와 대조하지 않고 그대로 수용 및 처리해 버리는 논리적 결함 체인입니다.
...
- **내용:** 판매자 재고/요금 관리 시 소유권 인가 검증이 결여되어, 요청 바디의 `propertyKey`를 경쟁 업체의 매물 고유 코드로 변조 송신하면 타인의 상품 가격을 조작하거나 강제 품절시킬 수 있습니다.

###  해결방안

결제 금액 파라미터를 그대로 수용하지 말고 백엔드가 DB에서 직접 원천 상품 가격을 가져와 직접 연산 대조해야 하며, 마일리지는 범위 필터를 적용해야 합니다. 또한 **전송된 식별자(memberId, propertyKey) 정보가 현재 요청 헤더의 JWT 토큰에서 인가 및 복호화하여 추출한 실제 로그인 사용자 식별자와 정확하게 일치하는지 상호 소유권 검증 로직을 구현**해야 합니다.
...
통합 설정부에서 임시 테스트용 로컬 호스트 허용 규칙을 완전히 제거하고, 승인된 공식 운영 도메인 대역만 요청 가능하도록 엄격히 한정해야 합니다.
...
전체 컴플라이언스 체계인 **8개 그룹 28개 항목** 대조판에 올려놓고 보니, 오늘의 치명적인 시나리오 덤프 결과물조차 **겨우 '1번 대분류 항목(입/출력 값 검증 부재)의 절반 정도'를 완료한 상태**임을 깨달았습니다.
