...
> **"이전 스프린트에서 정립한 1인 전 공정(End-to-End) 통합 개발 체제를 바탕으로 아르고스(Argus) 플랫폼의 3주 일정을 확정했습니다. 장대현 멘토님께 배포 완료된 온데(Onde) 플랫폼과 아르고스의 3단계 사양을 검증받고 실전 진단 방법론 강의를 수령했으며, 수립된 기준에 따라 온데에 최초 동적 스캔(DAST)을 걸어 Swagger 주입으로 API를 추적할 수 있는지, 500 응답이 버퍼 오버플로우로 잡히는 오탐이 어느 정도인지, Selenium 증적 캡처가 되는지까지 가능성을 점검했습니다."**
>
...
- **판단:** 서버가 비정상 입력에 대해 HTTP 500 또는 오류 페이지를 돌려준 것을, 스캐너가 치명적 오동작·버퍼 오버플로우 성공으로 오인한 오탐(False Positive)으로 보았습니다.
...
# 5. Next Step: 스캔 정책 정교화 및 SK 가이드라인 룰셋 매핑

최초 DAST로 API 추적·오탐 양상·실탐(6-1)·증적 캡처 가능성을 확인했으니, 다음부터는 결과를 쓸 수 있는 수준으로 규칙을 다듬습니다.

- **ZAP 스캔 정책(Scan Policy) 튜닝:** 500 응답 등 오탐을 걸러내고 실제 예외·오동작을 구분하는 규칙 수립.
