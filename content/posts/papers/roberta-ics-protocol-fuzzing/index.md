...
| 항목 | 명세 기반 | 랜덤 변이 기반 | 랜덤 변이 기반 (보정) | 제안 방식 (온도 **1.0~1.5**) |
| --- | --- | --- | --- | --- |
| **탐색 깊이** | 분기 커버리지 **30% 안팎** | 숫자는 비슷해 보여도 **절반 가까이 입구에서 떨어져** 서버에 닿는 입력이 적음 | — | **40%대**까지 상승 |
| **처리량** | 빠른 편 | — | 제안 Parallel과 **비슷하거나 조금 적음** | **Parallel** + 낮은 온도로 하루 **400만 건** 가까이. 속도·탐색 깊이를 함께 노릴 설정이 있음 |
| **형식 + 다양성** | 파싱은 통과, 유니크 **53%** | 파싱 실패 **62%**로 입구 탈락이 많음 | **Invalid 99%**, 읽기·쓰기를 거의 검사하지 못함 | 파싱 실패를 낮게 유지하면서 유니크 **90% 이상** |
| **의미 있는 변이 위치** | 본문만 건드림 | 헤더까지 깨뜨려 입구에서 막히기 쉬움 | 형식만 맞추고 의미 있는 탐색은 거의 없음 | **헤더는 안정적으로 두고 본문에서 예외를 유도** |
| **현장 맞춤** | 환경이 바뀌면 템플릿을 다시 짜야 함 | 환경이 바뀌면 규칙을 다시 짜야 함 | 환경이 바뀌면 규칙을 다시 짜야 함 | 학습 패킷만 바꿔도 기능 코드 분포가 달라져, **수작업 템플릿 없이** 그 환경에 맞는 퍼징 가능 |
...
![Fig.5 학습 데이터에 따른 기능 코드 분포 — CSS vs ICS](./fig5.png)
