...
이번 프로젝트는 **데이터 수집부터 실제 서비스 구현까지** 전 과정을 직접 겪어 볼 수 있는 시간이었습니다.

특히 처음 제대로 써 본 **Git · GitHub 버전 관리**는 코드를 올려 두는 일을 넘어, 작업 기록을 남기고 팀과 보조를 맞추는 법을 배우는 계기가 됐습니다. 각자 맡은 기능을 따로 만들고 하나로 합치는 과정에서, 여럿이 하는 프로젝트에 **약속과 규칙**이 왜 필요한지 몸으로 느꼈습니다. (그래서 미니 때 페이지가 갈라져 쌓인 빚을, 끝난 뒤 공통 모듈로 다시 묶은 흐름과도 이어집니다.)

떨어져 있던 **수집 엔진**, Pandas **전처리**, Streamlit **화면**을 한 서비스로 이어 보면서 전체 개발 흐름을 더 선명하게 볼 수 있었습니다.

처음 해 보는 일이 많아 서툰 점도 많았는데, 끝까지 믿고 이끌어 준 팀원들 덕분에 마칠 수 있었습니다. 함께 고생하며 많이 알려 준 팀원들에게 고맙습니다.
