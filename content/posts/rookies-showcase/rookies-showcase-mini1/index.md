...
| 추출 | 역할 |
|------|------|
| `paths` / `brand` / `pricing` | 경로·브랜드 색·단가·할인율 |
| `data_loader` | 캐시된 categorized 로드·정규화 |
| `filters` / `product_grid` | 필터 UI·카드·페이지네이션 |
| `html_safe` / `ui_css` | escape·URL allowlist, CSS 1회 주입 |
| `scraper/base` | 브랜드 raw 저장 보일러플레이트 |
| `theme_guide` | 다이어트·야식 공통 틀 (키워드 셋만 주입) |

`get_brand_color`·로컬 `get_data`·카드 HTML이 페이지마다 있던 자리를 걷고, 7-Eleven 스크래퍼에 **파일 전체가 이중으로 붙어 있던 복제**도 제거했습니다. 순 라인은 줄어드는 쪽(+931 / −1475 수준)이었습니다.
