...
상품명 키워드로 **제로/저당 · 고단백** 테마를 걸어 목록을 만드는 화면입니다. “제로”가 붙은 주류·생활용품처럼 **이름만 보고 잘못 걸리는 항목**은 제외 키워드로 걸러 내고, 남는 행만 공통 테마 가이드 UI(`theme_guide`)로 필터·정렬·페이지네이션을 합니다. 별도 영양 DB 없이 **상품명 키워드만으로** 다이어트 맥락의 행사 상품을 좁힙니다.
