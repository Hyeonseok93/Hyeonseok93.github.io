...
물가가 올라가면서 1+1 · 2+1만 골라 사는 소비가 흔해졌고, 1인 가구·학생층에서는 “싸게 사기”를 넘어 **가성비를 맞춰 쓰는** 수요가 커졌습니다. 단순 목록이 아니라 비교·예산·테마 같은 **맞춤 추천**까지 붙이면, 루키즈에서 배운 Streamlit·데이터 파이프라인을 한 제품 흐름으로 묶어 볼 수 있었습니다.
...
상품명 키워드로 **제로/저당 · 고단백** 테마를 걸어 목록을 만드는 화면입니다. “제로”가 붙은 주류·생활용품처럼 **이름만 보고 잘못 걸리는 항목**은 제외 키워드로 걸러 내고, 남는 행만 공통 테마 가이드 UI(`theme_guide`)로 필터·정렬·페이지 나눔을 합니다. 별도 영양 DB 없이 **상품명 키워드만으로** 다이어트 맥락의 행사 상품을 좁힙니다.
...
배치 마지막 단계(Selenium)로 모은 `official_event_news.csv`를 브랜드 필터·페이지 나눔으로 보여 줍니다. 상품 목록과 분리된 **공식 공지 피드**라서, 뉴스 수집이 실패해도 카탈로그 화면은 그대로 두고 소식만 어제 데이터를 유지합니다. 24시간 이내 글에는 NEW 뱃지를 붙입니다.
...
순서는 대략 **배치의 급한 버그 → 공통 모듈 → Streamlit·보안 → 나머지 정리**였습니다. 아래는 그중 **화면에 바로 보이던 문제**와 **구조·보안 쪽 판단**을 묶은 기록입니다.
...
## 배치의 급한 버그 — 어제 카탈로그를 지키게

화면보다 덜 보이지만, 데이터가 틀어지면 대시보드 전체가 흔들립니다. 점검에서 **당장 손봐야 했던** 것은 대략 세 가지였습니다.
...
| 카드 그리드·페이지 나눔 | `utils/product_grid` | `render_product_grid` / `paginate` |
...
예를 들어 전체 요약은 “필터 → 페이지 나눔 → 그리드”만 남기고, 다이어트·야식은 **테마 키워드만 다른 설정**으로 같은 `render_theme_guide`를 탑니다. 7-Eleven 스크래퍼의 **이중 복제(~73줄×2)** 도 한 구현으로 정리했습니다.
