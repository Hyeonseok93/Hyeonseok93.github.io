...
검색·예약·결제·피드·셀러/어드민까지, 서비스가 화면에서 어떻게 이어지는지 봅니다. (홈·추천 메인은 **1. 메인 화면** `fig1`과 동일합니다.)

### 1. 숙소 예약 페이지 (Stay Page) — `/`

목적지·체크인/아웃·인원·객실로 숙소를 검색하고, 숙소 소개·객실 정원·요금을 토글해 봅니다. 일정을 고르면 숙박 일수와 날짜별 단가로 객실별 합산 요금이 바뀌고, 객실·인원을 정해 결제 단계로 넘깁니다.
