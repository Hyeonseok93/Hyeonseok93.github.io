...
모집 → 지원 → 매칭 → 팀 게시판이 화면으로 이어집니다. (스크린샷은 `fig4` 이후 슬롯에 채울 예정입니다.)
...
프론트는 MSW로 먼저 흐름을 고정하고, 백엔드는 Entity·상태 전이·권한을 코드로 고정했습니다. 아직 ERD 이미지·화면 캡처는 이 글 슬롯에 맞춰 채울 예정이고, README에는 요약과 이 글로의 링크만 남겨 두었습니다.
