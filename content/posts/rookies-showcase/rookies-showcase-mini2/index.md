...
기술 스택은 User·Project 모두 `@ElementCollection`으로 `user_tech_stacks` / `project_tech_stacks`에 문자열 집합으로 둡니다. Soft Delete가 필요한 도메인은 `BaseEntity`의 `deleted_at` + `@SQLRestriction("deleted_at IS NULL")`를 공통 적용합니다.
...
일반 삭제 API는 `deleted_at`만 채웁니다. `@SQLRestriction` 때문에 일반 조회에서는 빠지고, 관리자 Thymeleaf 화면은 `findAllIncludingDeleted`로 Soft Delete 행까지 본 뒤 **복구(`deleted_at = null`)** 할 수 있습니다. 회원 삭제 시 소유 프로젝트도 함께 Soft Delete하고, 동작은 `AdminLog`에 남깁니다.
...
- **명세 그대로의 모킹 레이어** — `mocks/handlers.js`에 auth·projects·applications·board·comments 전 구간을 40개 핸들러로 깔았습니다. 단순히 고정 JSON을 뱉는 게 아니라, `localStorage`를 가짜 DB(`mock-db`)로 써서 로그인·모집글 CRUD·지원·수락/거절이 **상태를 유지하며** 돌도록 만들었습니다. 덕분에 백엔드 없이도 "지원하면 목록에 뜨고, 수락하면 멤버가 되는" 실제 시나리오를 로컬에서 검증할 수 있었습니다.
...
## 잘못된 입력은 저장 전에, 오류는 내부 정보 없이

검증이 요청 계층과 저장 계층 양쪽에 있었는데, 요청 DTO보다 엔티티 제약이 더 엄격한 필드가 남아 있었습니다. 지원 동기는 엔티티에서 10–500자였지만 요청 DTO에는 "비어 있지 않다"만 걸려 있어, 9자짜리 지원서가 요청 검증을 통과한 뒤 저장 시점에 터져 **500으로 돌아왔습니다**. 사용자 입력 문제인데 서버 오류처럼 보이는 응답이었습니다.

- **제약을 요청 경계로 끌어올림** — 지원 동기(10–500자), 모집글 제목(5–50자), 댓글(1–500자), 닉네임(2–10자)처럼 엔티티에만 있던 길이 제약을 요청 DTO에도 맞췄습니다. 참고 링크·연락처·본문에도 최대 길이를 두어, 잘못된 길이는 저장 전에 걸러지고 **어떤 필드가 왜 틀렸는지 담은 400**으로 돌아옵니다.
- **입력은 텍스트로만** — 제목·본문·지원 동기·댓글에 `<`, `>`가 들어오면 요청 단계에서 거부하고, 참고 링크는 `http/https` 형식을, 연락처는 제어문자를 함께 검사합니다. 관리자 화면에서 값을 텍스트로 렌더링하도록 바꾼 것과 더불어, **입력을 받는 순간부터** 마크업을 막는 방어를 한 겹 더 뒀습니다.

오류 응답이 내부 정보를 흘리지 않는지도 같은 흐름에서 정리했습니다.

- **응답에서 입력 원문 제거** — 검증 실패 응답이 틀린 값을 그대로 되돌려 주고 있어, 이메일·전화번호 같은 입력이 응답에 다시 실렸습니다. 필드명과 안내 문구만 남기고 입력 원문은 응답에서 뺐습니다.
- **예외를 일반화** — 스택 트레이스·예외 클래스·DB/SQL 메시지는 사용자 응답에 넣지 않고, 잘못된 JSON은 400, 없는 API는 404, 허용되지 않은 메서드는 405로 분류했습니다. 프론트도 서버가 정의한 안전한 메시지나 안내 문구만 노출하고, 임의의 에러 원문을 그대로 띄우지 않습니다.
...
최종 백엔드 결과는 **24 tests passed / 0 failed**입니다. 로컬 MariaDB나 `.env` 없이도 같은 테스트가 돌아가도록 만들었습니다.
...
프론트에는 Vitest + jsdom을 추가해 API 에러 추출, 에셋 URL, ID 정규화, 상태 계산, 빈 닉네임 Avatar 등을 **40개 단위 테스트**로 묶었습니다. 백엔드 24개 테스트와 함께 GitHub Actions에서 다음 순서로 자동 검증합니다.
...
최종 확인은 **백엔드 24/24**, **프론트 40/40**, ESLint 0 errors, 프로덕션 빌드 성공입니다.
