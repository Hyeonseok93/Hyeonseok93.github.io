...
NVD의 CVE와 OSV의 권고는 **서로 다른 ID·표현**으로 쌓입니다. 한쪽만 보면 “공식 CVE”와 “패키지 쪽 인텔”이 어떻게 맞물리는지 놓치기 쉽습니다. 같은 Explorer에서 소스를 넘나들며 검색하고, 연관 ID를 따라가며 **비교**할 수 있는 화면이 필요했습니다.

### 전체 그림을 보고 싶었다

개별 CVE 몇 건이 아니라, 카탈로그 **규모·심각도·소스 비중·성장·KEV**처럼 전체 분석이 보이길 바랐습니다. 대시보드 차트와 요약 지표는 “모아 둔 목록”을 넘어서 **한눈에 흐름을 읽게** 하려는 쪽에 가깝습니다.

### 수집부터 화면까지 한 줄로

비교·분석을 하려면 결국 데이터를 **받아 두고 → 표로 맞추고 → 집계해 보여 주는** 끝까지 가야 합니다. 그래서 bronze에 원본을 남기고 silver에서 정규화한 뒤 gold로 Explorer·대시보드를 채우도록 잡았고, 긴 파이프라인은 Roost에서 단계별로 돌리게 했습니다.
